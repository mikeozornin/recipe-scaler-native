...
3. Четыре status-баннера переходят на чтение `gate.isVisible` вместо прямой проверки `connectionState`.
4. Feature-gating (заблокированные кнопки, гейты изображений, `AssistantSheet`) остаётся мгновенным — там debounce привёл бы к бесполезным действиям в офлайне.
...
- Применять debounce к feature-gating местам (`AssistantSheet`, `allowsImageNetworkRefresh`, `RecipeDetailImageSection`, `ImportRecipeSheet`, `DataManagementView`, `YDocRecipeDetailView`, `DiscoverRecipeView`, `disabled`-кнопки в `ShoppingListShareSheet`).
- Менять web-контракт, Y.Doc schema, сервер.
- Добавлять новые UI-строки — используются существующие ключи (`account.offline.alert`, `account.public-profile.offline`).
...
### Status-баннеры → переходят на debounced gate (4 места)
...
- `AssistantSheet` — заменяет композер плейсхолдером `assistant.offline.description`; debounce привёл бы к send-ошибке.
...
1. **Given** приложение сворачивалось в фон с `.disconnected`, **When** пользователь разблокирует телефон и `scenePhase = .active`, **Then** в течение всего цикла `.disconnected → .connecting → .connected` ни один из четырёх status-баннеров не появляется.
...
Пользователь уходит в реальный офлайн (включает Airplane Mode, заходит в лифт без сети) дольше чем на 3 секунды. Все четыре status-баннера появляются синхронно — в Profile видна секция `wifi.slash` + `account.offline.alert`, в Telegram-секции и Shopping Share Sheet — `account.public-profile.offline`.
...
**Независимое тестирование**: включить Airplane Mode на >3с → баннеры появляются во всех четырёх местах.

**Acceptance Scenarios**:

1. **Given** приложение онлайн, **When** сеть пропадает и остаётся отсутствовать >3с, **Then** `OfflineBannerGate.isVisible` становится `true` и все четыре status-баннера появляются одновременно.
...
- **FR-008**: Четыре status-баннера ДОЛЖНЫ быть переведены на чтение `@Environment(OfflineBannerGate.self).isVisible`:
...
- **FR-009**: Feature-gating места (`AssistantSheet`, `allowsImageNetworkRefresh`, `RecipeDetailImageSection`, `disabled`-кнопки) ДОЛЖНЫ остаться на мгновенном чтении `connectionState.isConnected`/`connectionState == .connected` — gate и эти места теперь читают **один и тот же сигнал**, но gate добавляет debounce, а feature-gate реагирует мгновенно.
...
- **FR-012**: Новые UI-строки НЕ добавляются — используются существующие ключи (`account.offline.alert`, `account.public-profile.offline`). `lint-i18n.sh` остаётся зелёным.
...
- **SwiftUI views**: `AppShellView` (инжект gate, единственный writer: `connectionState` + `scenePhase`). `AccountView` (2 места), `TelegramConnectionView`, `ShoppingListShareSheet` читают `isVisible`. `AssistantSheet`, `ImportRecipeSheet`, `DataManagementView`, `RecipeListView`, `CollectionFolderView`, `RecipeDetailImageSection`, `YDocRecipeDetailView`, `DiscoverRecipeView` — НЕ читают gate (мгновенный feature-gating).
...
- **SC-001**: При разблокировке телефона ни один из четырёх status-баннеров не мелькает в окне `.disconnected → .connecting → .connected`.
- **SC-002**: Реальная потеря сети дольше 3с показывает все четыре status-баннера синхронно.
- **SC-003**: Возврат сети после показа мгновенно скрывает все четыре баннера (без 3с задержки).
- **SC-004**: Feature-gating места (`AssistantSheet`, `disabled`-кнопки, гейты изображений) реагируют мгновенно как и раньше — поведение не изменилось.
...
- Gate и feature-gating места (AssistantSheet, disabled-кнопки, allowsImageNetworkRefresh) читают **один и тот же сигнал** (`!connectionState.isConnected`), но gate добавляет debounce 3с, а feature-gate реагирует мгновенно. Это означает, что в первые 3с реального офлайна AssistantSheet скажет «офлайн», а status-баннер ещё нет — это сознательное UX-решение: action-screen должен мгновенно предотвратить бесполезное действие, а status-баннер не должен мелькать при разблокировке.
...
- 4 места status-баннеров исчерпывают все «мелькающие» UI-элементы; места типа `AssistantSheet` намеренно оставлены мгновенными (action-screen, лучше не давать бесполезных действий).
