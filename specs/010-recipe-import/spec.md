...
| US6 offline «Попробовать позже» | ✅ (2026-08-13) При `!isOnline` сегменты Text/Photo в segmented control disabled (`AppSegmentedControl.Segment.isDisabled`, dimmed, не тапаются). Если выбранным оказался disabled-сегмент — авто-fallback на `.file` (`onChange(isOnline)` при переходе в офлайн; `resetState()` при холодном открытии sheet в офлайне), через `applySystemMode(_:)` который НЕ сбрасывает `errorMessage`/`photoWarning`/`fileImportProgress` (no-wipe invariant; см. `onChange(of: mode)` guard на `isSystemModeChange`). Под segmented control рендерится баннер `import.offline-banner` (иконка `wifi.slash`). `mode == .file` работает офлайн (нативный JSON). `submit()` safety net (`import.offline-unavailable`) использует `isConnectedStrict` (строгий `.connected`). **Два уровня gating**: `isOnline` (UI, оптимистичный: `.connected ∪ .connecting ∪ .reconnecting`) управляет сегментами/баннером; `isConnectedStrict` (только `.connected`) передаётся в `NativeExportImportService`/`ThirdPartyRecipeImportService`/`submit()` pre-check — сервисы сами имеют `waitForConnection(timeout: 5)` grace window. Feature-gating мгновенный; `OfflineBannerGate` (debounce 3с, spec 066) намеренно не используется — это non-goal в 066. |
