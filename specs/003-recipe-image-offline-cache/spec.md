...
**Дата**: 2026-06-02  
**Статус**: реализовано, ручная приёмка пройдена (2026-06-02)  
...
После `YjsSyncService.refreshCollectionEntries` при подключении к серверу приложение ДОЛЖНО в фоне prefetch **превью и full** для записей с непустым `imageUrl`:

- до 3 рецептов параллельно;
- для каждого рецепта: сначала `preview`, затем `full` (последовательно, один in-flight на вариант).

При открытии деталки — дополнительный prefetch `full` для активного рецепта (если фоновая загрузка ещё не завершилась или `imageUrl` есть только в recipe document).
...
2. Офлайн (режим полёта): превью в списке и full в шапке деталки видны без запросов к API, если после последнего онлайн-sync коллекции фоновый prefetch успел завершиться.
3. Смена фото на вебе (новый `imageUrl`) → после reconnect превью и full обновляются.
4. Удаление фото (`imageUrl` пустой) → превью исчезает, файлы кэша (`_preview` и `_full`) удалены.
