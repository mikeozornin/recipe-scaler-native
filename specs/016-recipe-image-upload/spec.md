...
| US1 upload photo | ✅ |
| US2 delete image | ✅ |
| US3 list preview | ✅ (003) |
| US4 офлайн | ✅ |

## Контекст

**003** реализует download + disk cache + display. Пользователь на вебе может **загрузить** фото и **удалить** — через REST; `imageUrl` в Y.Doc обновляется с сервера/sync.
...
- Загрузка изображения по URL (`POST .../image-from-url`) — есть на вебе, в iOS не реализуется
