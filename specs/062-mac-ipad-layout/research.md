...
**Дата**: 2026-08-09 (актуализация исходного исследования) | **Spec**: [spec.md](./spec.md)
...
Перед Фазой A приложить в `specs/062-mac-ipad-layout/screenshots/`:
...
## R9 — Platform feasibility probe (2026-08-09)

### Decision

Native macOS target технически подтверждён: универсальный Yrs XCFramework
установлен, `RecipeScalerMac` создан и generic build проходит. Catalyst/source
fallback больше не нужен как основной маршрут; следующий gate — runtime QA.

### Evidence

- `Package.swift` объявляет `.iOS(.v17)` и `.macOS(.v14)`; `ZIPFoundation` добавлена в SwiftPM target `RecipeScalerCore` и синхронизирована с Xcode project.
- `swift build --target RecipeScalerCore --triple arm64-apple-macosx --scratch-path /private/tmp/recipe-scaler-core-macos-probe` завершился успешно.
- Rust stable с targets `aarch64-apple-darwin` и `x86_64-apple-darwin` собрал y-crdt `v0.26.0`; `scripts/build-yrs-xcframework.sh` завершился успешно во временный каталог `/private/tmp/recipe-scaler-yrs-macos-probe`.
- Output содержит `ios-arm64`, `ios-arm64_x86_64-simulator` и `macos-arm64_x86_64`; `lipo -info` подтверждает ожидаемые архитектуры.

### Boundary

`Frameworks/YrsXCFramework.xcframework` содержит проверенный macOS slice, а
Xcode линковка `RecipeScalerMac` и iOS regression build проходят. Simulator/UI
runtime проверка остаётся отдельным gate, так как доступного iPhone runtime в
текущем окружении нет.
