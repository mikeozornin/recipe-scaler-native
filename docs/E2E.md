...
Аутентификация: симуляторный DEBUG-build применяет `DebugSimulatorAutoLogin` (`userId` + `device_token`) в `AuthService.init` → приложение автоматически логинится под `f088233a-…` против `recipe-scaler.ru` с Bearer (spec 041). Override токена: `DEBUG_DEVICE_TOKEN`. Отключить: `-DisableDebugAutoLogin=1`.
