...
Аутентификация: симуляторный DEBUG-build использует `DebugSimulatorAutoLogin` (`userId` + `device_token`) → приложение автоматически логинится под `f088233a-…` против `recipe-scaler.ru` с Bearer (spec 041). Override токена: `DEBUG_DEVICE_TOKEN`. Отключить: `-DisableDebugAutoLogin=1`.
