...
Если симулятор с другим именем — подставь доступный из `xcrun simctl list devices available` или укажи `id=<UDID>`. При ошибках — исправить и пересобрать. Опционально после успешного build: `rtk xcodebuild … test` или сценарий из `scripts/verify-<feature>.sh`, если правки затрагивают фичу с verify-скриптом.
