...
| 27 | ✅ | `035-architecture-di-observable` (Phase B): введён `RecipeScalerNative/App/AppContainer.swift` (`@MainActor @Observable final class`) как single source of truth для 12 сервисов; `RecipeScalerNativeApp.init` конструирует контейнер, провайдит через `.appEnvironment(container)`; `RecipeScalerNative/App/AppEnvironment.swift` (`@Entry EnvironmentValues` для контейнера + per-service keys); `RecipeScalerNative/App/TimerEventBridge.swift` разрывает циклический callback `TimerSyncService.sendTimerEvent ↔ YjsSyncService.emitTimerEvent` (weak refs с обеих сторон); side-effects из `YjsSyncService.start` (APIClient/TimerSync/ImageCache observers, LiveActivity lookup) перенесены в `AppContainer.bootstrap(userId:)`; `ContentView` упрощён до тонкого view; `.shared` на 12 сервисах превращён в shim → `AppContainer.shared.<service>` с `Standalone` fallback для previews/tests; OS-фасады (`SharedAuthStore`/`AppGroup`/`TimerSnapshotStore`/`APIClient.shared`) оставлены; новые тесты `AppContainerTests` |
| 28 | ✅ | `035-architecture-di-observable` (Phase C): мигрированы 7 классов на `@Observable` макрос — `YjsSyncService` (15 `@Published`), `RemindersSyncService` (2), `SpotlightIndexer` (vestigial), `RecipeListViewModel` (+`final`), `DescriptionEditorBridge` (6), `DescriptionEditorChromeState` (3, делегирует в bound `bridge`); vestigial `: ObservableObject` удалён с `APIClient`; 19 view consumers — `@EnvironmentObject` → `@Environment(YjsSyncService.self)`, 4 view'а DescriptionEditorBridge/ChromeState — `@StateObject`/`@ObservedObject` → `@State`/`@Bindable`; Combine-подписки (`SpotlightIndexer`, `RemindersSyncService`) переписаны на `withObservationTracking`; новые тесты `ObservableMigrationTests` |
...
#### 27. ~~**[Architecture]** Нет composition root с DI; 14 синглтонов тянутся друг в друга~~ ✅ Исправлено (2026-06-19, spec 035)
...
- ~~**Description**: `ContentView.init` — фактический composition root, вручную связывает 4 из ~14 сервисов; остальные через `.shared`. Cross-service wiring спрятан внутри методов: `YjsSyncService.start` (858) конфигурит `TimerSyncService.shared`, затем (863) назначает `TimerSyncService.shared.sendTimerEvent = { ... self? ... }` — циклический callback. 79 call sites ссылаются на «большую четвёрку» синглтонов.~~ — **сделано**: введён `RecipeScalerNative/App/AppContainer.swift` (`@MainActor @Observable final class`) как single source of truth для 12 сервисов. `RecipeScalerNativeApp.init` конструирует контейнер; `WindowGroup` получает его через `.appEnvironment(container)`. Скрытое связывание внутри `YjsSyncService.start` (APIClient/TimerSync/ImageCache observers) перенесено в `AppContainer.bootstrap(userId:)`. Циклический callback `TimerSyncService.sendTimerEvent ↔ YjsSyncService.emitTimerEvent` вынесен в `RecipeScalerNative/App/TimerEventBridge.swift` (weak refs с обеих сторон). `ContentView` упрощён до тонкого view, читающего контейнер из environment. `.shared` на 12 сервисах превращён в shim, который форвардит в `AppContainer.shared.<service>` если контейнер построен, иначе в lazily-instantiated `Standalone`. OS-фасады (`SharedAuthStore`, `AppGroup`, `TimerSnapshotStore`, `APIClient.shared`) оставлены как есть — это cross-process IPC для extensions. Build green (3 scheme), new `AppContainerTests` — green.

#### 28. ~~**[Architecture]** Смешаны `ObservableObject` и `@Observable` в слое сервисов/view-моделей~~ ✅ Исправлено (2026-06-19, spec 035)
...
- ~~**Description**: Кодбейз в середине миграции. Самый большой state-holder (`YjsSyncService`) — на старом фреймворке, форсит `@StateObject`/`@EnvironmentObject`. `ContentView` смешивает оба на одном view.~~ — **сделано**: мигрированы все 7 классов на `@Observable` макрос — `YjsSyncService` (15 `@Published` → `private(set) var`), `RemindersSyncService` (2 `@Published`), `SpotlightIndexer` (vestigial), `RecipeListViewModel` (+`final`), `DescriptionEditorBridge` (6 `@Published`), `DescriptionEditorChromeState` (3 `@Published`, теперь делегирует focus/ready в bound `bridge`). У `APIClient` удалена vestigial `: ObservableObject` conformance (0 `@Published`). 19 view consumers переписаны с `@EnvironmentObject` → `@Environment(YjsSyncService.self)`, 4 view'а для DescriptionEditorBridge/ChromeState — с `@StateObject`/`@ObservedObject` → `@State`/`@Bindable`. Combine-подписки в `SpotlightIndexer` и `RemindersSyncService` переписаны на `withObservationTracking`. Build green (3 scheme), new `ObservableMigrationTests` — green.
